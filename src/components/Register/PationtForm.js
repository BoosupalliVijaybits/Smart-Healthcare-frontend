...
import { genderList, pationtRegister } from "../../Data/DummyJson";
import { dropdown, no_view, view } from "../../assets/image";
import { toast } from "react-toastify";
import { useRegisterPatientMutation } from "../../Data/Api/formapi";
import { usePationtregisterMutation } from "../../Data/Api/api";

const PationtForm = ({ setLoad }) => {
  ...
  // Api
  const [pationtRegisterApi] = usePationtregisterMutation();
      ...
      pationtRegisterApi(payload)
        ...
        .catch((err) => {
          ...
          if (err?.data?.message) {
            ...
            toast.error("Error Code:", err?.status);
          }
        })
        .finally(() => {
          setLoad(false);
                    ...
                    {genderList?.map((item) => {
                      return (
                        <button
                          ...
                          className="w-100 d-flex ac-js gap-1 border-0 bg-transparent"
                        >
                          <div
                            className={`${
