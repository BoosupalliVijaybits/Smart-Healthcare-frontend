...
import { doctorRegister, genderList } from "../../Data/DummyJson";
import { dropdown, no_view, view } from "../../assets/image";
import { toast } from "react-toastify";
import { useDoctorregisterMutation } from "../../Data/Api/api";
import { useRegisterDoctorMutation } from "../../Data/Api/formapi";

const DoctorForm = ({ setLoad }) => {
  ...
  // Api
  // const [doctorRegisterApi] = useDoctorregisterMutation();
  const [doctorRegisterApi] = useRegisterDoctorMutation();
    ...
    switch (field) {
      case "licenseNumber":
        if (!stringValue) {
          ...
          errorMsg = "License Number must be at least 6 characters long!";
        }
        break;
      case "specialization":
        if (!stringValue) {
      ...
      case "qualifications":
        if (!stringValue) {
          errorMsg = "Qualifications is required!";
        }
        break;
      case "experience":
        if (!stringValue) {
  ...
  const submitHandler = () => {
    ...
    if (isValid) {
      ...
      const payload = {
        ...
        password: formFeald?.confirmPassword,
        firstName: formFeald?.firstName,
        lastName: formFeald?.lastName,
        phoneNumber: formFeald?.phoneNumber,
        clinicName: formFeald?.clinicname,
        qualification: formFeald?.qualifications,
        clinicAddress: formFeald?.clinicaddress,
        gender: formFeald?.gender,
        specialization: formFeald?.specialization,
        licenseNumber: formFeald?.licenseNumber,
        experience: formFeald?.experience,
        dateOfBirth: formFeald?.dateofbirth,
      };
      console.log("payload", payload);
      // const formData = new FormData();
      // formData.append("data", JSON.stringify(payload));
      // console.log("payloaformdatad", payload, formData);

      doctorRegisterApi(payload)
        .unwrap()
        .then((res) => {
          console.log("res", res);
          res?.message
        ...
        .catch((err) => {
          console.log("Err", err);
          toast.error(
            err?.data?.message || err?.data?.licenseNumber || err?.status
          );
        })
        .finally(() => {
          setLoad(false);
                  ...
                  <div className="drop-cont d-flex w-100 ac-js">
                    ...
                    <div
                      className={`${
                        ...
                        (item?.formFeald === "Maritalstatus" &&
                          droptoggle?.Maritalstatus)
