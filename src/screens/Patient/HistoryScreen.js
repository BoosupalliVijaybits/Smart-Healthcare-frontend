  ...
  const priscribFun = () => {
    ...
    mypriscription(user_id)
      ...
      .then((res) => {
        ...
        const completedPrescriptions = res.filter(
          (item) => item.status === "COMPLETED"
        );

        setPriscriptions(completedPrescriptions || []);
      })
      .catch((err) => {
        console.log("err", err);
      })
  ...
  return (
    <>
      ...
      {priscriptions?.length == 0 ? (
        <EmptyScreen img={notfound} content={"History Not Found"} />
      ) : (
              ...
              <tbody>
                {currentRows.map((prescription, index) => (
                  <tr key={index}>
                    ...
                    <td>{prescription?.appointmentResponse?.id}</td>
